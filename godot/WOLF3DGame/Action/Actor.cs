...
namespace WOLF3D.WOLF3DGame.Action
{
	...
	&& Assets.DigiSoundSafe(digiSound) is AudioStreamSample audioStreamSample)
		...
		public Actor T_Path(float delta = 0f)
		{
			...
			{
				Recenter();
				if (Main.ActionRoom.Level.GameMap.IsWithinMap(X, Z)
					&& Assets.Turns.TryGetValue(Main.ActionRoom.Level.GameMap.GetObjectData((ushort)X, (ushort)Z), out Direction8 direction))
					Direction = direction;
				if (Direction == null)
		...
		public Actor T_Schabb(float delta = 0f) => this;//TODO impelement T_Schabb
		public static void T_SchabbThrow(Actor actor, float delta = 0f) => actor.T_SchabbThrow(delta);
		public Actor T_SchabbThrow(float delta = 0f) => this;//TODO impelement T_SchabbThrow
		public static void T_Gift(Actor actor, float delta = 0f) => actor.T_Gift(delta);
		public Actor T_Gift(float delta = 0f) => this;//TODO impelement T_Gift
		public static void T_GiftThrow(Actor actor, float delta = 0f) => actor.T_GiftThrow(delta);
		public Actor T_GiftThrow(float delta = 0f) => this;//TODO impelement T_GiftThrow
		public static void T_Fat(Actor actor, float delta = 0f) => actor.T_Fat(delta);
		public Actor T_Fat(float delta = 0f) => this;//TODO impelement T_Fat
		...
		{
			Transform = new Transform(Transform.basis, new Vector3(Assets.CenterSquare(X), 0f, Assets.CenterSquare(Z)));
			return this;
		}
		public Actor Kill()
		{
